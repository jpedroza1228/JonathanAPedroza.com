...
I will be the first to state that I am not an expert in the field of conducting psychometric models, Bayesian networks, or Bayesian analyses, **but** I have been struggling to find any blog posts about conducting a bayes net with latent variables that uses [Stan](https://mc-stan.org/). The purpose of this post is to walk through Stan and some bayes net terminology to get a basic understanding of some psychometric models conducted using Bayesian inference.
...
\
For these models, I'll be using the ECPE dataset from the [edmdata package in R](https://github.com/tmsalab/edmdata/). This dataset has 28 items for 2,922 students with 3 latent attributes/variables. A 1 for each item indicates whether a student got the item correct and a 0 indicates they got the item incorrect.
...
The next model is going to extend from the previous model with edges between the latent attributes in a linear pattern. So attribute 1 will be the parent to attribute 2 and then attribute 2 will be the parent to attribute 3.
...
A convergent bayes net will have different edges between the latent attributes. Instead of attribute 3 having one parent, like with the linear bayes net example, a convergent bayes net will have both attribute 1 and attribute 2 as parents.
...
The parent and child nodes for a divergent bayes net will differ from the two previous models, where attribute 1 will now be the parent to both attribute 2 and attribute 3.
...
Below are DINA models through a Bayesian Network framework. The DINA model assumes that each student has mastered all skills in order to correctly respond to an assessment item. See [here](https://mc-stan.org/learn-stan/case-studies/dina_independent.html) for an excellent post about the DINA model. This model can also be expanded on to model as a bayes net. The main difference for the bayes net is that we will be creating a delta parameter from priors on student mastery of each attribute. In the post highlighting a DINA model, a similar matrix is created outside of stan (included in the `data` argument in the stan code referred to as `xi`) that is the global attribute mastery indicator.

Similar to the LCDM models, the DINA models can also include parent-child edges between the latent attributes, which will have an impact on the delta parameter. The other models are listed below.
