...
This post will be part two of the my series on Bayesian Networks, or bayes net as I'll refer to them for the rest of this post. As stated in the introduction post (see \[here\]()), I'll be starting off with a Log-Linear Cognitive Diagnostic Model (LCDM), which is a general purpose model that allows for different probabilities between the latent classes. The stan code included in this post will be for a LCDM. At the end of this blog post, I'll also be including more in-depth information with the inclusion of formulas.
...
library(reticulate)
use_python(here::here("website_venv/bin/python3.12"))
